...
function ChucklePostAI(AI_option) {
    ...
    function MAIN(AI_option) {
        ...
        if (AI_option.eliminate && AI_option.eliminate.length && AI_option.eliminate.some(function(item) { return currentURL.includes(item); })) {
            console.log("Post-Summary-AI 已排除当前页面(黑名单)");
            return;
        }
        if (AI_option.whitelist && AI_option.whitelist.length && !AI_option.whitelist.some(function(item) { return currentURL.includes(item); })) {
        ...
        var post_title = document.querySelector(AI_option.title_el) ? document.querySelector(AI_option.title_el).textContent : document.title;
        ...
        var post_ai_box = document.createElement('div');
        ...
        var explanation = document.querySelector('.ai-explanation');
        var post_ai = document.querySelector('.post-ai');
        var ai_btn_item = document.querySelectorAll('.ai-btn-item');
        var ai_str = '';
        var ai_str_length = '';
        var delay_init = 600;
        var i = 0;
        var j = 0;
        var speed = AI_option.speed || 20;
        var character_speed = speed * 7.5;
        var sto = [];
        var elapsed = 0;
        var completeGenerate = false;
        ...
        function animate(timestamp) {
            ...
            if (elapsed >= speed) {
                ...
                if (i < ai_str_length - 1) {
                    var char = ai_str.charAt(i + 1);
                    var delay = /[,.，。!?！？]/.test(char) ? character_speed : speed;
        ...
        async function aiInspiration() {
            resetAI();
            var response = await getAIResponse("你是一个灵感发生器，给用户提供有意思的灵感，不要超过100字，不要分段，不要分点，不要换行");
            if (response) {
                startAI(response);
            }
        }
        ...
        async function aiGenerateAbstract() {
            ...
            var content = getTextContent(ele);
            console.log("获取到的文章内容:", content.substring(0, 200) + "...");
            var prompt = '请根据以下文章内容生成一个简洁的摘要，不要超过200字，不要换行，不要提出建议或评论，只需总结文章主要内容。文章标题和内容如下：' + content;
        ...
        async function getAIResponse(prompt) {
            ...
            var apiUrl = "https://deepseek.example.com";
        ...
        function getTextContent(element) {
            var totalLength = AI_option.total_length || 3000;
            var content = '文章标题：' + post_title + '。文章内容：' + getText(element);
            return content;
        }
        ...
        function getText(element) {
            ...
            var excludeClasses = AI_option.exclude ? AI_option.exclude : ['highlight', 'Copyright-Notice', 'post-ai', 'post-series'];
            if (!excludeClasses.includes('post-ai')) { excludeClasses.push('post-ai'); }
            var excludeTags = ['script', 'style', 'iframe', 'embed', 'video', 'audio', 'img', 'svg'];
            ...
            if (element.innerText) {
                ...
                for (var node of element.childNodes) {
                    if (node.nodeType === Node.TEXT_NODE) {
                        ...
                        var hasExcludeTag = excludeTags.includes(node.tagName.toLowerCase());
            ...
            if (textContent.length < 100) {
                ...
                var commonSelectors = ['.post-content', '.entry-content', '.content', 'article', '#article-container', '.article-content', '.post-body'];
                for (var selector of commonSelectors) {
                    var fallbackElement = document.querySelector(selector);
                    if (fallbackElement && fallbackElement !== element) {
                        console.log("尝试使用备选选择器:", selector);
                        var fallbackContent = fallbackElement.innerText || getText(fallbackElement);
        ...
        function extractString(str, totalLength) {
            totalLength = Math.min(totalLength || 1000, 5000);
            if (str.length <= totalLength) { return str; }

            var firstPart = str.substring(0, Math.floor(totalLength * 0.5));
            var lastPart = str.substring(str.length - Math.floor(totalLength * 0.5));
        ...
        function getArticleElements() {
            ...
            for (var selector of selectors) {
                ...
                if (element) {
                    ...
                    var textLength = element.innerText ? element.innerText.length : 0;
            ...
            var allElements = document.querySelectorAll('div, section, main, article');
            var maxTextElement = null;
            var maxTextLength = 0;
            ...
            for (var el of allElements) {
                ...
                var tlen = el.innerText ? el.innerText.length : 0;
                if (tlen > maxTextLength && tlen > 200) {
                    maxTextLength = tlen;
                    maxTextElement = el;
                }
            }
        ...
        async function ai_init() {
            ...
            var funArr = [aiIntroduce, aiInspiration, aiGenerateAbstract];

            ai_btn_item.forEach(function(item, index) {
